...
import { createHttpServer } from '../src/server/nativeServer.js';
...
describe('4. End-to-End Leakage Recovery & Financial Ledger Scenario', () => {
...
describe('5. Full Native API Server & REST Endpoints', () => {
...
describe('6. Adversarial Chaos & High-Concurrency Edge Cases', () => {
  it('handles high concurrency burst of 100 leads without data corruption', () => {
    const store = new LeadEventStore();
    ...
    assert.equal(metrics.total_inbound_leads, 100);
    assert.equal(metrics.active_leads_count, 100);
    assert.ok(metrics.pipeline_value_total_usd > 50000);
  });

  it('rejects malformed payload missing customer phone number', () => {
    assert.throws(() => {
      normalizeAndEnrichLead({
        channel: 'MISSED_CALL',
